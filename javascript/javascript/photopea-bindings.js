...
function onPhotopeaLoaded(iframe) {
    ...
    // Special handling for txt2img (better for reForge)
    setTimeout(() => {
        const txt2imgGallery = gradioApp().querySelector("#txt2img_gallery");
        if (txt2imgGallery) {
            createSendToPhotopeaButtonForTxt2img(txt2imgGallery);
        }
    }, 800); // slightly longer delay for reForge
}
...
function createSendToPhotopeaButton(queryId, gallery) {
    const buttonId = `${queryId}_open_in_photopea`;
...
// Creating button patch.
function createSendToPhotopeaButtonForTxt2img(gallery) {
    const buttonId = "image_buttons_txt2img_open_in_photopea";

    if (gradioApp().getElementById(buttonId)) return;

    const existingButton = gradioApp().querySelector("#image_buttons_txt2img button") 
                        || gradioApp().querySelector("#txt2img_gallery button");

    if (!existingButton) {
        console.warn("[Photopea] Could not find base button for txt2img");
        return;
    }
...
function openImageInPhotopea(originGallery) {
    let outgoingImg = null;

    if (originGallery) {
        outgoingImg = originGallery.querySelector("img");
    }

    if (!outgoingImg) {
        const txt2imgGallery = gradioApp().querySelector("#txt2img_gallery");
        if (txt2imgGallery) {
            outgoingImg = txt2imgGallery.querySelector("img");
        }
    }

    if (!outgoingImg) {
        outgoingImg = gradioApp().querySelector("#txt2img_gallery img");
    }

    if (!outgoingImg) {
        outgoingImg = gradioApp().querySelector("#img2img_gallery img");
    }

    if (!outgoingImg) {
        console.error("[Photopea] Could not find image");
        alert("Could not find the image in txt2img. Try clicking the image first.");
        return;
    }
    ...
    postMessageToPhotopea(getPhotopeaScriptString(getActiveDocumentSize)).then((response) => {
        ...
        const imageSizeMatches = !(outgoingImg.naturalWidth > activeDocSize[0] || 
            ...
            postMessageToPhotopea(`app.open("${imageData}", null, ${imageSizeMatches});`, "*")
                .then(() => {
                    if (imageSizeMatches) {
                        ...
                        postMessageToPhotopea(`alert("New document created because image is bigger");`, "*");
                    }
                });
        });
    });
